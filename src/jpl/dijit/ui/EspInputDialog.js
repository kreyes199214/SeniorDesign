...
define([
    ...
    'dojo/text!../templates/EspInputDialog.html'
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
        ...
        doEsp: function(evt)
        {
          ...
          if(this.config.espServiceURL !== "")
          {
            var url = this.config.espServiceURL;
            url += '?topbc=' + top;
            url += '&leftbc=' + left;
            url += '&bottombc=' + bottom;
            url += '&rightbc=' + right;
            url += '&email=' + emailAddr;
              ...
              //hack for Bach to just make this work.
            if(this.proj === this.config.projection.N_POLE) {
                url += '&proj=NP&projString='+this.proj;
            } else if(this.proj === this.config.projection.S_POLE) {
                url += '&proj=SP&projString='+this.proj;
            } else {
                url += 'proj=EQ&projString='+this.proj;
            }

            var token = this.getToken();
            token = this.fixedEncodeURIComponent(token);
            url += '&iPlanetDirectoryPro=' + token;
            console.log('doEsp():: url = ' + url);
          ...
          else
          {
            topic.publish(BrowserEvent.prototype.SHOW_ALERT, {
              title: 'Esp',
              content: 'No espService service available',
              size: 'sm'
            });
          }
        ...
        doProcessResult: function(result)
        {
          ...
          topic.publish(BrowserEvent.prototype.SHOW_ALERT, {
            title: 'Surface ElectroStatic Potential',
            content: content,
            size: 'sm'
          });
        },
