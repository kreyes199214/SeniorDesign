define([
    ...
    return declare([WMTSLayer],{
        ...
        onLoad: function() {
            ...
            this.layerInfo.tileInfo.lods = this.addMoreLODS(this.layerInfo.tileInfo.lods, 19);
        },

        _getLodFromTileMatrix: function (tileMatrixXML, wkid, level, c) {
            if (wkid != 0)
                return this.inherited(arguments);
            else {
                //polar projection
                var identifier = this._getTagValues("Identifier", tileMatrixXML)[0];
                var scale = this._getTagValues("ScaleDenominator", tileMatrixXML)[0];
                if (1 < scale.split("E").length) {
                    var scaleSplit = scale.split("E");
                    scale = scaleSplit[0] * Math.pow(10, scaleSplit[1]);
                } else {
                    scale = parseFloat(scale);
                }
                var resolution = scale * 0.00028;
                var obj = {level: level, levelValue: identifier, scale: 1.058267716535433 * scale, resolution: resolution};
                return obj
            }
        },
        ...
        _tileLoadHandler: function(a) {
            ...
            // do/undo what is done in the parent function _popTile _tilePopPop
        ...
        addMoreLODS: function(originalLODs, totalLevels) {
